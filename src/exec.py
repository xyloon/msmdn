...
def main():
    ...
    parser.add_argument('browser_driver', type=str, help='chrome driver location')
    ...
    browser = None
    try:
        browser = Browser(args.browser_driver)
        parsed_page = browser.read_page(args.page_num)
        ...
        information_for_pages = dict((page_num, {}) for page_num, _ in parsed_page['page_info'])
        information_for_pages[args.page_num].update(extract_title_image_dict(parsed_page))

        for page_num, info_dict in information_for_pages.items():
            if not info_dict:
                info_dict.update(
                    extract_title_image_dict(data_dict=browser.choose_option(page_num))
                )

        pprint(information_for_pages)
        for one_issue in information_for_pages.values():
            # dir check
...
def extract_title_image_dict(data_dict):
    return dict(((k, v) for k, v in data_dict.items() if k in ('title', 'images')))
